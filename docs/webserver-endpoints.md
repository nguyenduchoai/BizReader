...
available while BizReader is in File Transfer or Calibre Wireless mode.
...
Examples use `bizreader.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://bizreader.local/api/status
```
...
```bash
curl "http://bizreader.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://bizreader.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://bizreader.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://bizreader.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://bizreader.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://bizreader.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://bizreader.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://bizreader.local/delete
```
...
```bash
curl http://bizreader.local/api/settings
```
...
curl -X POST \
  ...
  http://bizreader.local/api/settings
```
...
```bash
curl http://bizreader.local/api/fonts
```
...
curl -X POST \
  ...
  http://bizreader.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://bizreader.local/api/fonts/delete
```
...
```bash
curl http://bizreader.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://bizreader.local/api/opds/delete
```
...
```bash
curl http://bizreader.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://bizreader.local/api/wifi/delete
```
...
- `bizreader.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `BizReader`.
